...
# review asks for something short and without a trailing full stop.
#
# It deliberately no longer matches the root command's own Short string, which
# says "Query recorded Kubernetes state changes". krew-index review asked for the
# word out, and it is right about its own context: every entry in that index is a
# kubectl plugin, so "Kubernetes" spends a scarce column on the one thing a reader
# of that list already knows. `kuberecord --help` is read without that framing —
# often by somebody who reached the standalone binary and no cluster at all — and
# there the word is the first useful thing in the sentence. Two audiences, two
# lines; keeping them identical would mean serving one of them worse.
cat <<EOF
...
spec:
  ...
  shortDescription: Query recorded state changes
  description: |
  ...
  caveats: |
    ...
    krew does not install that one; the project's own installation instructions
    say where to get it.
