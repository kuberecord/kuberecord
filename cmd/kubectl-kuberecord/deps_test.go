...
// operatorPackages are the packages the CLI binary must never be able to reach (D20).
//
	...
	for _, dep := range goListDeps(t) {
		for _, forbidden := range operatorPackages {
			if dep == forbidden || strings.HasPrefix(dep, forbidden+"/") {
				t.Errorf("cmd/kubectl-kuberecord reaches %q (via the closure of its imports); "+
...
// certifies nothing. This asserts the binary really is wired to the contract it
// is supposed to be a client of, so that the test above is measuring a boundary
// that something is actually pressing against.
//
// It is repeated here rather than left to internal/cli because the binary is
// where the boundary finally has to hold: this package could acquire an operator
// import of its own, and its closure is the one that becomes the shipped
// artifact.
func TestTheReadPlaneContractIsActuallyReached(t *testing.T) {
	if !slices.Contains(goListDeps(t), readPlaneContract) {
		t.Errorf("cmd/kubectl-kuberecord no longer reaches %q, so TestNoOperatorPackagesInTransitiveDeps "+
