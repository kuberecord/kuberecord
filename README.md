...
| [`docs/EVENTS.md`](docs/EVENTS.md) | Kubernetes Events end to end, for the rule author deciding whether to add `kind: Event`. Why capture is scope-wide and correlation happens when you read, the `eventFilter` axes and which of them reach the API server, the `count`-bump amplifier that no filter bounds and the `coalesceWindow` that does, how to size a rule against it, and what `--with-events` and `--events-only` do at the other end. |
