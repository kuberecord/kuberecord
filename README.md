...
- **Redaction before hashing** — a sink's `spec.policy.redaction` sets a floor and a rule's `spec.extraRedaction` adds to it (never subtracts), scrubbing configured values out of every object *after* normalization and *before* hashing. Because the hash, the diff baseline and the stored payload are all functions of the redacted content, a value that is scrubbed cannot leak through a diff delta or be ground out of the `sha256` column — two states differing only in a redacted value are indistinguishable and deduplicate away. `kubectl.kubernetes.io/last-applied-configuration` is scrubbed unconditionally, because `kubectl apply` copies the whole object into it. See [`docs/SCHEMA.md`](docs/SCHEMA.md#redaction).
...
- `spec.policy.redaction` (sink) and `spec.extraRedaction` (rule) list values to scrub before hashing. Each entry sets exactly one of `fieldPath` (dot segments plus an optional `[*]` array wildcard, e.g. `spec.template.spec.containers[*].env[*].value`) or `annotation` (one annotation key, for keys whose dots and slashes a `fieldPath` cannot spell). The syntax is pattern-checked and the exactly-one rule is CEL, so a malformed path is rejected at admission rather than discovered at stream time. Redaction is additive in every direction and is **not** a way to stream `v1/Secret`. See [`docs/SCHEMA.md`](docs/SCHEMA.md#redaction).
