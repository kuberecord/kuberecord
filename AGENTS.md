...
**In kuberecord (Task 2.4):** both options ship, but the chart is **hand-written**
at `deploy/charts/kuberecord`, not generated by the `helm/v2-alpha` plugin — do not
run `kubebuilder edit --plugins=helm/v2-alpha`, it would scaffold a second,
competing chart. Use `make helm-sync` (refresh the chart's copies of the CRDs and
RBAC presets), `make helm-lint`, `make helm-kubeconform`, `make verify-packaging`
and `make test-e2e-helm`; `dist/install.yaml` is committed and reproduced by
`make build-installer` with no arguments. `deploy/charts/kuberecord/README.md`
documents every value, and `test/chart` asserts the chart and
`kustomize build config/default` install the same operator.
