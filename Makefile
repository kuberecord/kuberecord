...
#
# The formula is staged *before* the up-to-date check, and that ordering is the
# whole of the check's correctness rather than a tidying of it. `git diff` compares
# the working tree against the index and, like every plumbing command that reads
# the index, it does not see an untracked file at all — so on a tap that does not
# yet carry the formula it reports no difference, the guard concludes there is
# nothing to push, and the job goes green having pushed nothing. That is precisely
# backwards: the guard exists to make a re-run idempotent, and the one case it got
# wrong was the first push into a new tap, where there is the most to do and the
# least to notice. Staging first and asking `--cached` compares against HEAD
# instead, where a file that is absent from the last commit is a difference —
# which is what "does the tap already carry this?" was always asking.
...
release-brew-push: ## Push this release's formula to BREW_TAP_REPO (needs HOMEBREW_TAP_TOKEN).
	...
	*-*) \
	...
	if git diff --cached --quiet -- "$(BREW_FORMULA_PATH)"; then \
