...
version is plain semver, so the artifact's tag is `0.4.0` where the operator's is
`v0.4.0`. The two always agree otherwise: the chart is **not** versioned
...
helm pull oci://ghcr.io/kuberecord/charts/kuberecord --version 0.4.0 --untar
kubectl apply -f kuberecord/crds/

helm upgrade kuberecord oci://ghcr.io/kuberecord/charts/kuberecord \
  --version 0.4.0 --namespace kuberecord-system
```
