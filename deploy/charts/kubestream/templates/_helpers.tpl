...
conventional release name `kubestream`, both paths install
`kubestream-controller-manager`, `kubestream-metrics-reader`,
`kubestream-watcher-core-workloads` and so on. That is not cosmetic: it is what
...
{{- define "kubestream.managerName" -}}
{{- printf "%s-controller-manager" (include "kubestream.fullname" .) | trunc 63 | trimSuffix "-" }}
{{- end }}

{{- define "kubestream.chart" -}}
...
*/}}
{{- define "kubestream.selectorLabels" -}}
control-plane: controller-manager
app.kubernetes.io/name: {{ include "kubestream.name" . }}
{{- end }}

{{/* Common labels, carried by every object the chart renders. */}}
{{- define "kubestream.labels" -}}
{{ include "kubestream.selectorLabels" . }}
app.kubernetes.io/instance: {{ .Release.Name }}
app.kubernetes.io/version: {{ .Chart.AppVersion | quote }}
app.kubernetes.io/managed-by: {{ .Release.Service }}
helm.sh/chart: {{ include "kubestream.chart" . }}
{{- with .Values.extraLabels }}
{{ toYaml . }}
{{- end }}
{{- end }}

{{- define "kubestream.serviceAccountName" -}}
{{- if .Values.serviceAccount.create }}
{{- default (include "kubestream.managerName" .) .Values.serviceAccount.name }}
{{- else }}
...
*/}}
{{- define "kubestream.validateValues" -}}
{{- if and (gt (int .Values.replicaCount) 1) (not .Values.leaderElection.enabled) }}
{{- fail "kubestream: replicaCount > 1 requires leaderElection.enabled=true — two active operators would double-write every row" }}
{{- end }}
{{- if and .Values.createDefaultSink (not .Values.defaultSink.connection.addr) }}
{{- fail "kubestream: createDefaultSink=true requires defaultSink.connection.addr (host:9000 of your ClickHouse)" }}
{{- end }}
{{- if and .Values.createDefaultSink (not .Values.defaultSink.connection.credentialsSecretRef.name) }}
{{- fail "kubestream: createDefaultSink=true requires defaultSink.connection.credentialsSecretRef.name — create the Secret yourself; the chart never templates a password" }}
{{- end }}
{{- end }}
