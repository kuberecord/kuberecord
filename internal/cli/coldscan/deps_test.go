...
//
// The list is repeated here rather than shared with the CLI's other packages
// because Task 11.8 split one package into several, and an assertion that did
// not travel with the split would leave the packages it no longer covers guarded
// by nothing. This package asks an engine for its capabilities and its scan estimate, so it reaches the read plane directly.
	...
	for _, dep := range goListDeps(t) {
		for _, forbidden := range operatorPackages {
			if dep == forbidden || strings.HasPrefix(dep, forbidden+"/") {
				t.Errorf("internal/cli/coldscan reaches %q (via the closure of its imports); "+
...
// certifies nothing. This asserts the package really is wired to the contract it
// is supposed to be a client of, so that the test above is measuring a boundary
// that something is actually pressing against.
func TestTheReadPlaneContractIsActuallyReached(t *testing.T) {
	if !slices.Contains(goListDeps(t), readPlaneContract) {
		t.Errorf("internal/cli/coldscan no longer reaches %q, so TestNoOperatorPackagesInTransitiveDeps "+
