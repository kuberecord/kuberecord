...
// by nothing. This is the exit contract itself, at the bottom of the order; it reaches the read plane only for query.ErrNoCoverage.
	...
	for _, dep := range goListDeps(t) {
		for _, forbidden := range operatorPackages {
			if dep == forbidden || strings.HasPrefix(dep, forbidden+"/") {
				t.Errorf("internal/cli/exit reaches %q (via the closure of its imports); "+
...
func TestTheReadPlaneContractIsActuallyReached(t *testing.T) {
	if !slices.Contains(goListDeps(t), readPlaneContract) {
		t.Errorf("internal/cli/exit no longer reaches %q, so TestNoOperatorPackagesInTransitiveDeps "+
