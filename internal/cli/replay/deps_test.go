...
// by nothing. This package replays recorded changes, so it reaches the read plane directly.
	...
	for _, dep := range goListDeps(t) {
		for _, forbidden := range operatorPackages {
			if dep == forbidden || strings.HasPrefix(dep, forbidden+"/") {
				t.Errorf("internal/cli/replay reaches %q (via the closure of its imports); "+
...
func TestTheReadPlaneContractIsActuallyReached(t *testing.T) {
	if !slices.Contains(goListDeps(t), readPlaneContract) {
		t.Errorf("internal/cli/replay no longer reaches %q, so TestNoOperatorPackagesInTransitiveDeps "+
