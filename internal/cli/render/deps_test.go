...
// by nothing. It renders the read plane's types, which is the only dependency it has here.
	...
	for _, dep := range goListDeps(t) {
		for _, forbidden := range operatorPackages {
			if dep == forbidden || strings.HasPrefix(dep, forbidden+"/") {
				t.Errorf("internal/cli/render reaches %q (via the closure of its imports); "+
...
func TestTheReadPlaneContractIsActuallyReached(t *testing.T) {
	if !slices.Contains(goListDeps(t), readPlaneContract) {
		t.Errorf("internal/cli/render no longer reaches %q, so TestNoOperatorPackagesInTransitiveDeps "+
