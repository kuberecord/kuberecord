...
func run(t *testing.T, args ...string) (stdout, stderr string, code int) {
	...
	code = cli.Run(append([]string{cli.StandaloneName}, args...), io)
...
func TestConfigSetProfileWritesWhatItWasTold(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("LoadConfig of what the command wrote: %v", err)
	}
	profile, ok := cfg.Profiles[profileProd]
	if !ok {
		t.Fatalf("the file holds no profile named prod: %+v", cfg)
	}
	if profile.Backend != cli.BackendClickHouse || profile.ClickHouse == nil {
	...
	if _, stderr, code = run(t, "config", "set-profile", "archive",
		"--backend", "s3", "--bucket", "acme-audit", "--prefix", "kuberecord"); code != cli.ExitSuccess {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if code != cli.ExitUsageError {
				t.Errorf("exit code %d, want %d for a malformed invocation", code, cli.ExitUsageError)
			}
...
`)
	...
	if _, stderr, code := run(t, "config", "use-profile", "archive"); code != cli.ExitSuccess {
	...
	if code != cli.ExitUsageError {
		t.Errorf("exit code %d, want %d for a profile that does not exist", code, cli.ExitUsageError)
	}
...
`)
	...
	if !strings.Contains(stdout, "apiVersion: "+cli.ConfigAPIVersion) {
	...
	t.Run("a missing file views as an empty configuration", func(t *testing.T) {
		...
		if !strings.Contains(stdout, "kind: "+cli.ConfigKind) {
...
//
// Cobra's own unknown-command error is a plain one, which ExitCodeFor reads as a
