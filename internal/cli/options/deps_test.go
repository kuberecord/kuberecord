...
// by nothing. It reaches the read plane through internal/cli/render, which formats the instants DescribeWindow prints.
	...
	for _, dep := range goListDeps(t) {
		for _, forbidden := range operatorPackages {
			if dep == forbidden || strings.HasPrefix(dep, forbidden+"/") {
				t.Errorf("internal/cli/options reaches %q (via the closure of its imports); "+
...
func TestTheReadPlaneContractIsActuallyReached(t *testing.T) {
	if !slices.Contains(goListDeps(t), readPlaneContract) {
		t.Errorf("internal/cli/options no longer reaches %q, so TestNoOperatorPackagesInTransitiveDeps "+
