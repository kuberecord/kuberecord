...
// by nothing. This package constructs query engines, so it reaches the read plane directly.
	...
	for _, dep := range goListDeps(t) {
		for _, forbidden := range operatorPackages {
			if dep == forbidden || strings.HasPrefix(dep, forbidden+"/") {
				t.Errorf("internal/cli/resolve reaches %q (via the closure of its imports); "+
...
func TestTheReadPlaneContractIsActuallyReached(t *testing.T) {
	if !slices.Contains(goListDeps(t), readPlaneContract) {
		t.Errorf("internal/cli/resolve no longer reaches %q, so TestNoOperatorPackagesInTransitiveDeps "+
