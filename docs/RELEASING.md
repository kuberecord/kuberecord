...
| A **cosign signature** on `checksums.txt` | `sign-blob`, because a file on a Release page has no digest a registry could name. One signature therefore covers every attached asset — the CLI archives, the install artifacts, the krew manifest, the Homebrew formula and both SBOMs — since each is a line in that file. From v0.3.0 (Task 12.1). |
