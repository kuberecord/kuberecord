...
| `kuberecord.yaml` | The krew plugin manifest: one `uri` and `sha256` per platform, rendered from [`.krew.yaml`](../.krew.yaml) against the archives above by `hack/krew-manifest.sh`. This is the file submitted to [kubernetes-sigs/krew-index](https://github.com/kubernetes-sigs/krew-index), and `kubectl krew install --manifest-url` takes it directly. From v0.3.0 onward. |
...
7. **Check that the krew-index pull request appeared.** The workflow's `krew` job
   asks the release bot to open it; nobody has to run anything. What that costs
   is that an automation nobody checks is an automation nobody notices failing,
   so look once:

   ```sh
   gh pr list --repo kubernetes-sigs/krew-index --search kuberecord --state all --limit 5
   ```

   The run's own summary says whether the bot reported success, and prints
   `make krew-index-pr` if it did not. Two cases where there will be no PR and
   that is correct: a prerelease tag, and the very first submission of the plugin
   — see below.
...
**The manifest has one definition**, and it is [`.krew.yaml`](../.krew.yaml) at
the repository root. That is the file to edit — never the rendered copy. Two
things read it:

- `hack/krew-manifest.sh` renders it against the archives `make release-cli` just
  packaged, hashing each one locally, and that is the `kuberecord.yaml` attached
  to the release.
- the bot renders the same file on tag push, downloading those published archives
  to hash them, and submits what it gets.

A template beside a generator would be two descriptions of one artefact, drifting
silently until krew-index carried a document nothing here had ever printed. The
renderer refuses anything the bot does not also provide — the accepted template
language is `{{ .TagName }}` and `{{addURIAndSha "<url>" .TagName }}` and nothing
else — and it refuses a platform mismatch in either direction, so a template that
named an archive `make build-cli` does not produce fails on the pull request that
introduced it rather than on somebody's `krew install`.

Four things follow, and all four are worth knowing before you tag:
...
- **A prerelease is never submitted.** krew-index carries the one version
  ...
  it did.
- **It cannot run before the release is published.** The bot downloads every
  archive the manifest names in order to hash it, and reads the Release itself to
  see whether it is a prerelease. So the job `needs: publish`, and krew-index's
  own CI does the same fetch again on arrival.
- **Merging is external, and slow for anything that is not a bump.** It does not
  block the release, and it should not: the archives, the Homebrew tap and
  `go install` all work the moment the tag is published.

**`make krew-index-pr` is kept as the fallback.** It downloads the published
manifest, re-checks every URI against the bytes GitHub is serving, forks
krew-index and opens the PR as you. Reach for it in three cases: the first
submission, a bot outage — the run summary prints the command when the step fails
— and a release whose manifest changed shape enough to want a human on the pull
request.

### The trust decision behind the bot

Worth stating plainly rather than leaving implied, because it is a third party in
the release path:

- `rajatjindal/krew-release-bot` is maintained by an individual, and it opens pull
  requests on this project's behalf. krew's developer guide recommends it, and the
  krew maintainers reviewing this plugin's submission recommended it specifically
  — the decision was deliberate rather than incidental.
- **It holds no secret of ours.** It renders the template on the runner, posts the
  result to its own service, and that service raises the PR through its own GitHub
  App. The `krew` job is granted `contents: read`, enough to check this repository
  out and nothing more. No token this repository can issue could write to
  krew-index anyway.
- **It is pinned to a commit**, like every other third-party action in the
  workflow. One residual: it is a Docker action whose own `image:` is a mutable
  tag (`ghcr.io/rajatjindal/krew-release-bot:v0.0.50`), so the pin fixes the
  action's definition, not the container it runs.
- **It changes nothing about the evidence.** Signing, provenance, the SBOMs and
  `checksums.txt` are untouched by any of this: the bot submits a manifest
  pointing at assets that were already signed and already attested.

The generation and checking targets, all of which a rehearsal runs:

```sh
make release-krew-manifest RELEASE_VERSION=v0.3.0   # .krew.yaml → dist/release/kuberecord.yaml
...
make release-krew-manifest RELEASE_VERSION=v0.2.0    # .krew.yaml → dist/release/kuberecord.yaml
...
make krew-index-pr RELEASE_VERSION=v0.2.0            # the fallback PR against kubernetes-sigs/krew-index
```
